...
std::vector<GenericTopicSubscription::Ptr> RvizViewer::create_subscriptions(
  ...
  aligned_points_pub =
    node.create_publisher<sensor_msgs::msg::PointCloud2>("~/aligned_points",
                                                         10);

  points_corrected_pub =
    node.create_publisher<sensor_msgs::msg::PointCloud2>("~/points_corrected",
                                                         10);
  ...
  pose_corrected_pub =
    node.create_publisher<geometry_msgs::msg::PoseStamped>("~/pose_corrected",
                                                           10);
...
void RvizViewer::set_callbacks() {
  ...
  OdometryEstimationCallbacks::on_update_new_frame.add(
    [this](const EstimationFrame::ConstPtr& new_frame) {
      odometry_new_frame(new_frame, true);
    });
...
void RvizViewer::odometry_new_frame(const EstimationFrame::ConstPtr& new_frame,
  ...
  if (publish_tf) {
    ...
    if (base_frame_id == imu_frame_id) {
      ...
      try {
        const auto trans_imu_base =
          tf_buffer->lookupTransform(imu_frame_id,
                                     base_frame_id,
                                     from_sec(new_frame->stamp));
  ...
  if (points_pub->get_subscription_count()) {
    ...
    logger->debug("published points (stamp={} num_points={})",
                  new_frame->stamp,
                  new_frame->frame->size());
  }

  auto& aligned_points_pub =
    !corrected ? this->aligned_points_pub : this->aligned_points_corrected_pub;
...
void RvizViewer::globalmap_on_update_submaps(
  ...
  invoke([this, latest_submap, submap_poses] {
    ...
    logger->warn(
      "Publishing global map is computationally demanding and not recommended");
    ...
    for (int i = 0; i < this->submaps.size(); i++) {
      ...
      std::transform(submap->points,
                     submap->points + submap->size(),
                     merged->points + begin,
