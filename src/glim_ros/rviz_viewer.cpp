...
RvizViewer::create_subscriptions(rclcpp::Node &node) {
  ...
  points_corrected_pub = node.create_publisher<sensor_msgs::msg::PointCloud2>(
      "~/points_corrected", 10);
  aligned_points_corrected_pub =
      node.create_publisher<sensor_msgs::msg::PointCloud2>(
          "~/aligned_points_corrected", 10);
  ...
  rmw_qos_profile_t map_qos_profile = {
      ...
      false};
  rclcpp::QoS map_qos(
      rclcpp::QoSInitialization(map_qos_profile.history, map_qos_profile.depth),
      map_qos_profile);
  map_pub =
      node.create_publisher<sensor_msgs::msg::PointCloud2>("~/map", map_qos);
  odom_pub = node.create_publisher<nav_msgs::msg::Odometry>("~/odom", 10);
  pose_pub =
      node.create_publisher<geometry_msgs::msg::PoseStamped>("~/pose", 10);

  odom_corrected_pub =
      node.create_publisher<nav_msgs::msg::Odometry>("~/odom_corrected", 10);
  pose_corrected_pub = node.create_publisher<geometry_msgs::msg::PoseStamped>(
      "~/pose_corrected", 10);
...
void RvizViewer::odometry_new_frame(const EstimationFrame::ConstPtr &new_frame,
    ...
    if (base_frame_id == imu_frame_id) {
      ...
      try {
        const auto trans_imu_base = tf_buffer->lookupTransform(
            imu_frame_id, base_frame_id, from_sec(new_frame->stamp));
        ...
        logger->warn(
            ...
            imu_frame_id, base_frame_id, stamp.sec, stamp.nanosec, e.what());
      }
    }
  ...
  if (points_pub->get_subscription_count()) {
    ...
    case FrameID::LIDAR: frame_id = lidar_frame_id; break;
    case FrameID::IMU: frame_id = imu_frame_id; break;
    case FrameID::WORLD: frame_id = map_frame_id; break;
    }
  ...
  auto &aligned_points_pub = !corrected ? this->aligned_points_pub
                                        : this->aligned_points_corrected_pub;
...
void RvizViewer::globalmap_on_update_submaps(
  ...
  invoke([this, latest_submap, submap_poses] {
    ...
    logger->warn(
        "Publishing global map is computationally demanding and not "
        "recommended");
